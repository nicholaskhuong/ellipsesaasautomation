...
public class IssueRecordAnInabilityNoBinSingleItemNon_QtyTracked_Step11 extends BaseTestCase {
	...
	public void showPickTasks() {
		...
		screenAction.waitObjVisible(driver, By.id(PickTasksPageDefinition.STOCK_CODE_ID), 3);
		...
		Assert.assertEquals(stockCode.get(0).getAttribute("value"), "STC67A");
		Assert.assertEquals(binCode.get(0).getAttribute("value"), "BIN1");
		Assert.assertEquals(quantityToIssue.get(0).getAttribute("value"), "50");
		Assert.assertEquals(unitOfIssue.get(0).getAttribute("value"), "EA");
		Assert.assertEquals(documentNumber.get(0).getAttribute("value"),"B01147");
	...
	public void clickPARTIALButton() {
		screenAction.clickBtnByIndex(By.id(PickTasksPageDefinition.PARTIAL_PICK_BUTTON_ID), 0);
	...
	@Test(description = "Tick in CheckBox on Close Pick Task page", dependsOnMethods = "showPickTasksDetails", alwaysRun = true)
	...
	public void selectReasonInDropDown() {
		screenAction.waitObjVisible(driver, By.cssSelector(PickTasksDetailsPageDefinition.INABILITY_REASON_CODE_ID), 6);
		driver.findElement(By.cssSelector(PickTasksDetailsPageDefinition.INABILITY_REASON_CODE_ID)).sendKeys("0001 Inability - No Discrepancy");
	}
	
	@Test(description = "Click PICK Button", dependsOnMethods = "selectReasonInDropDown", alwaysRun = true)
	public void clickPICK1Button() {
		screenAction.clickBtn(By.cssSelector(PickTasksDetailsPageDefinition.PICK_BUTTON_ID));
	}
	
	@Test(description = "Input quantity pick in Pick Task Details", dependsOnMethods = "clickPICK1Button", alwaysRun = true)
