...
public class QuickReceiptFullQuantity_NormalPO_OwnedSOH_Step12_13 extends BaseTestCase {
	...
	public void inputPurchaseOrderNumber() {
		...
		purchaseOrderNumber .sendKeys("P05054");
	}
	...
	public void clickSEARCHButton() {
		...
		actionBtn.clickBtn(By.id(POSearchPageDefinition.SEARCH_BUTTON13_ID));
	}
	...
	public void displayDetailItems () {
		...
		Assert.assertEquals(stockCode.get(0).getAttribute("value"),"STC024");
	...
	public void inputValueOnItemDetailPage() {
		...
		Assert.assertEquals(documentNumber.get(0).getAttribute("value"), "P05054");
		receiptReference.get(0).clear();
		receiptReference.get(0).sendKeys("04MAYCHAO");
	}
	...
	@Test(description = "Display Message On Item Page", dependsOnMethods="clickRECEIVEButton",alwaysRun = true)
	public void displayMessageItemPage() {
		List<WebElement> lismg =driver.findElements(By.cssSelector(ItemDetailPageDefinition.MESSAGE_ERROR_TEXT_ID));
		Assert.assertEquals(lismg.get(0).getText(),"(ERROR) 3140.E0073: Bin Code Is Required");
		lismg.get(0).click();
	}
	
	@Test(description = "Input New Bin On Item Page", dependsOnMethods="displayMessageItemPage",alwaysRun = true)
	public void inputNewBinOnItemPage() {
		driver.findElement(By.id(ItemDetailPageDefinition.NEW_BIN_ID)).sendKeys("BIN5");
	}
	
	@Test(description = "Click Receive Button after input Bin On Item Detail Page", dependsOnMethods="inputNewBinOnItemPage",alwaysRun = true)
	public void clickRECEIVEButtonAfterInputBin() {
		ScreenAction actionBtn = new ScreenAction(driver);
		actionBtn.clickBtn(By.cssSelector(ItemDetailPageDefinition.RECEIVE_BUTTON_ID));
	}
	
	@Test(description = "Display Message On PO Item Page", dependsOnMethods="clickRECEIVEButtonAfterInputBin",alwaysRun = true)
	public void displayMessageOnPOItemPage() {
		//Assert.assertEquals(driver.findElement(By.xpath(POItemsPageDefinition.NEW_PO_ITEMS_TEXT_ID)).getText(), "PO Items"); // Still error
//		ScreenAction action = new ScreenAction(driver);
//		action.assertMessage((driver.findElement(By.cssSelector(POItemsPageDefinition.MESSAGE_TEXT_ID)).getText()),"The search did not return any results\n(INFO) CORE.E06004: Action successfully completed.\n(INFO) 3140.I0464: Successfully receipted purchase order. P05052001");
