...
public class QuickReceipt_NormalPO_MultiplesItems_Steps02_05 extends BaseTestCase {
	...
	public void inputPurchaseOrderNumber() {
		...
		purchaseOrderNumber.sendKeys("P05100");
	}
	...
	//System setup purchase order only 3 items
	...
	public void displayDetailItems() {
		...
		Assert.assertEquals(stockCode.get(0).getAttribute("value"), "STC86");
		...
		Assert.assertEquals(documentItem.get(1).getAttribute("value"), "002");
		Assert.assertEquals(partNumber.get(1).getAttribute("value"), "");
		Assert.assertEquals(stockCode.get(1).getAttribute("value"), "STC86");
		Assert.assertEquals(description.get(1).getAttribute("value"), "STOCK CODE;");
		Assert.assertEquals(outstandingQuantityUOP.get(1).getAttribute("value"), "20.0");
		Assert.assertEquals(unitOfPurchase.get(1).getAttribute("value"), "EA");
		Assert.assertEquals(outstandingQuantityUOI.get(1).getAttribute("value"), "20.0");
		Assert.assertEquals(unitOfIssue.get(1).getAttribute("value"), "EA");
		
		Assert.assertEquals(documentItem.get(2).getAttribute("value"), "003");
		Assert.assertEquals(partNumber.get(2).getAttribute("value"), "");
		Assert.assertEquals(stockCode.get(2).getAttribute("value"), "STC86");
		Assert.assertEquals(description.get(2).getAttribute("value"), "STOCK CODE;");
		Assert.assertEquals(outstandingQuantityUOP.get(2).getAttribute("value"), "30.0");
		Assert.assertEquals(unitOfPurchase.get(2).getAttribute("value"), "EA");
		Assert.assertEquals(outstandingQuantityUOI.get(2).getAttribute("value"), "30.0");
	...
	public void clickPULLButton() {
		screenAction.clickBtnByIndex(By.cssSelector(POItemsPageDefinition.FULL_BUTTON_ID), 2);
	}
	...
	@Test(description = "Input Receipt Reference On Item Detail Page", dependsOnMethods = "displayItemDetailItems", alwaysRun = true)
