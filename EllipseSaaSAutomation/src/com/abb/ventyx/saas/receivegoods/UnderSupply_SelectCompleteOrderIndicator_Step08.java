...
public class UnderSupply_SelectCompleteOrderIndicator_Step08 extends BaseTestCase {
	...
	public void inputPurchaseOrderNumber() {
		...
		purchaseOrderNumber.sendKeys("P05107");
	}
	...
	public void inputPurchaseOrderItemNumber() {
		...
		purchaseOrderItemNumber.sendKeys("1");
	...
	public void displayDetailItems() {
		...
		Assert.assertEquals(outstandingQuantityUOP.get(0).getAttribute("value"), "1.0");
		Assert.assertEquals(unitOfPurchase.get(0).getAttribute("value"), "EA");
		Assert.assertEquals(outstandingQuantityUOI.get(0).getAttribute("value"), "1.0");
	...
	public void inputReceiptQtyOnItemDetailPage() {
		...
		receiptReference.sendKeys("20000");
	}
	
	@Test(description = "Input New Bin On Item Detail Page", dependsOnMethods = "displayItemDetailItems", alwaysRun = true)
	public void inputNewBinOnItemDetailPage() {
		screenAction.waitObjVisible(driver,By.id(ItemDetailPageDefinition.NEW_BIN_CODE_ID), 3);
		WebElement receiptReference = driver.findElement(By.id(ItemDetailPageDefinition.NEW_BIN_CODE_ID));
		receiptReference.clear();
		receiptReference.sendKeys("BIN1");
	}
	
	@Test(description = "Input Number of Label On Item Detail Page", dependsOnMethods = "inputNewBinOnItemDetailPage", alwaysRun = true)
	public void inputNumberofLabelOnItemDetailPage() {
		screenAction.waitObjVisible(driver,By.id(ItemDetailPageDefinition.LABEL_QUANTITY_ID), 3);
		WebElement receiptReference = driver.findElement(By.id(ItemDetailPageDefinition.LABEL_QUANTITY_ID));
		receiptReference.clear();
		receiptReference.sendKeys("300000");
	}
	
	@Test(description = "Input Receipt Reference On Item Detail Page", dependsOnMethods = "inputNumberofLabelOnItemDetailPage", alwaysRun = true)
	...
	public void displayMessageErrorOnPOItemPage() {
		screenAction.waitObjVisible(driver, By.cssSelector(ItemDetailPageDefinition.MESSAGE_ERROR_TEXT_ID), 3);	
		Assert.assertEquals(driver.findElement(By.cssSelector(ItemDetailPageDefinition.MESSAGE_ERROR_TEXT_ID)).getText(),"(ERROR) 3140.E0114: Over Supply Entered but not Confirmed");
		driver.findElement(By.cssSelector(ItemDetailPageDefinition.MESSAGE_ERROR_TEXT_ID)).click();
	}
	
	@Test(description = "Tick CheckBox Accept Over supply on Item Detail page", dependsOnMethods = "displayMessageErrorOnPOItemPage", alwaysRun = true)
	public void tickAcceptOversupplyCheckBox() {
		screenAction.waitObjVisible(driver, By.cssSelector("[type='checkbox']"), 5);
		driver.findElement(By.xpath("//*[@id='fgReceiveGoodsDetails']/div[21]/div[2]/span/input[@type='checkbox']")).click();
	}
	
	// TODO: Need to change Allow Issue in Excess of Hoding = Y in System (MSO00R)
//	@Test(description = "Click Receive Button On Item Detail Page", dependsOnMethods = "tickAcceptOversupplyCheckBox", alwaysRun = true)
//	public void clickRECEIVEButtonaAfterTickAcceptOverSupply() {
//		screenAction.clickBtn(By.id(ItemDetailPageDefinition.RECEIVE_ID));
//		screenAction.waitObjVisible(driver, By.id(ItemDetailPageDefinition.RECEIVE_ID), 3);
//	}
//	
//	@Test(description = "Display Message On PO Item Page", dependsOnMethods = "clickRECEIVEButtonaAfterTickAcceptOverSupply", alwaysRun = true)
